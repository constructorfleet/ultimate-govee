...
class GoveeEffectService:
    ...
    async def _make_req(self, url: str, headers: Dict[str, Any], payload: Optional[Dict[str, Any]] = None):
        import inspect
    ...
    async def get_light_effects(self, auth_state: Any, model: str, goods_type: int, device_id: str) -> List[LightEffect]:
        if not getattr(auth_state, "token", auth_state):
            logger.error(f"Unable to retrieve Light Effects for {device_id}: not authenticated")
            return []
        try:
            ...
            for e in effects_raw:
                effects.append(LightEffect(name=e.get("name"), id=e.get("id"), op_str_base64=e.get("opStr")))
            return effects
