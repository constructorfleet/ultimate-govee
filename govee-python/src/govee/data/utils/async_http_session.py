...
def _make_default_async_session(max_retries: int = 3, backoff_factor: float = 0.2, timeout: float = 10.0) -> Callable[..., Any]:
    if not _HAVE_HTTPX:
        # Provide a placeholder that raises early to make the error obvious.
        async def _no_httpx(*_args, **_kwargs):
            raise ImportError("httpx is required for the async HTTP session; install httpx or provide a custom session")

        return _no_httpx

    client = httpx.AsyncClient(timeout=timeout)

    async def session(method: str, url: str, headers: Optional[Dict[str, str]] = None, params: Optional[Dict[str, Any]] = None, json: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        for attempt in range(attempts):
            try:
                ...
                try:
                    ...
                    body = {}
                return {"status": resp.status_code, "statusText": resp.reason_phrase, "data": body}
