...
class Request:
    ...
    async def get(
        self, as_type: Optional[Type[T]] = None, save_to_file: Optional[str] = None
    ) -> Dict[str, Any]:
        # Use session if provided, otherwise raise (callers should inject fake session in tests)
        if not self._session:
            raise RuntimeError("no session provided")
        resp = self._session(
            method="GET", url=self.url, headers=self.headers, params=self.payload
        )
        status = int(resp.get("status", 200))
        if status != 200:
            raise ApiError(
                resp.get("statusText", "HTTP error"),
                ApiResponseStatus(
                    statusCode=status, message=resp.get("statusText", "")
                ),
            )
    ...
    async def post(
        ...
        if not self._session:
            raise RuntimeError("no session provided")
        resp = self._session(
            method="POST", url=self.url, headers=self.headers, json=self.payload
        )
        ...
        if http_status != 200:
            raise ApiError(
                resp.get("statusText", "HTTP error"),
                ApiResponseStatus(
                    statusCode=http_status, message=resp.get("statusText", "")
                ),
            )
        ...
        if data_status != 200:
            raise ApiError(
                content.get("message", "Unexpected Error"),
                ApiResponseStatus(
                    statusCode=data_status, message=content.get("message", "")
                ),
            )
        if save_to_file:
            with open(save_to_file, "w", encoding="utf-8") as f:
