...
def state_to_command_control_lock(identifier: list[int], next_state: bool) -> Dict[str, Any]:
    # return structure similar to TypeScript stateToCommand: command list and status
    return {
        'command': [
            {
                'data': {
                    'command': [[2] + identifier + ([1] if next_state else [0])]
                }
            }
        ],
        'status': {
            'op': {
                'command': [[1 if next_state else 0]]
            }
        }
    }
