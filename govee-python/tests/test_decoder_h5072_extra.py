...
def test_h5072__tempc_postproc():
    ...
    val = decoder_lib.Decoder.decode(
        device,
        ["value_from_hex_string", "manufacturerdata", 6, 6, False, False],
        ["&", 8388607, "/", 10000, "*", -1],
    )
...
def test_h5072_hum_postproc_examples():
    ...
    val1 = decoder_lib.Decoder.decode(
        device1,
        ["value_from_hex_string", "manufacturerdata", 6, 6, False, False],
        ["&", 8388607, "%", 1000, "/", 10],
    )
    ...
    val2 = decoder_lib.Decoder.decode(
        device2,
        ["value_from_hex_string", "manufacturerdata", 6, 6, False, False],
        ["&", 8388607, "%", 1000, "/", 10],
    )
    ...
    val3 = decoder_lib.Decoder.decode(
        device3,
        ["value_from_hex_string", "manufacturerdata", 6, 6, False, False],
        ["&", 8388607, "%", 1000, "/", 10],
    )
...
def test_h5072_batt_examples():
    ...
    for inp, expected in inputs:
        val = decoder_lib.Decoder.decode(
            {"manufacturerData": inp},
            ["value_from_hex_string", "manufacturerdata", 12, 2, False, False],
            [],
        )
