...
def test_property_matches_bits_and_inverse():
    ...
    assert (
        property_matches({"manufacturerData": "abcdef"}, ["manufacturerdata", 2, "zz"])
        is False
    )
...
def test_account_configuration_helpers():
    ...
    ah = configuration.govee_authenticated_headers(
        {"clientId": "cid", "accessToken": "tok"}, "3"
    )
...
def test_lan_parse_export_and_receiver_socket_bind():
    ...
    dummy = DummySocket()
    rsock = ReceiverSocket(
        socket=dummy, config={"bindAddress": "0.0.0.0", "receiverPort": 12345}
    )
