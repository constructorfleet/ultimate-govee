...
def test_unsupported_decoder_returns_none():
    ...
    res = Decoder.decode(
        {"manufacturerData": "abcd"},
        ["unsupported_decoder", "manufacturerdata", 0, 2, False, False],
    )
    assert res is None
...
def test_decode_hex_string_invalid_characters():
    ...
    res = Decoder.decode(
        {"manufacturerData": "zzzz"},
        ["value_from_hex_string", "manufacturerdata", 0, 4, False, False],
    )
    assert res is None
...
async def test_spec_top_level_condition_blocks_spec(monkeypatch):
    ...
    peripheral = {
        "advertisement": {"manufacturer_data": b"H123|"},
        "address": "AA:BB:CC:DD",
    }
...
async def test_iot_manager_called_with_expected_device_info(monkeypatch):
    ...
    peripheral = {
        "advertisement": {"manufacturer_data": b"H999|"},
        "address": "11:22:33:44",
    }
    decoded = await svc.decode_device(peripheral)
