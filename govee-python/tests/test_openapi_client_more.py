...
from govee.data.openapi.client import AsyncOpenApiClient, OpenApiNotFound, OpenApiTimeout
...
def test_get_iot_credentials_404_raises():
    ...
    client = AsyncOpenApiClient(base_url="http://api", session=fake_session, retries=1, timeout=1)

    with pytest.raises(OpenApiNotFound):
        asyncio.get_event_loop().run_until_complete(client.get_iot_credentials('device-123'))
...
def test_get_iot_credentials_timeout_raised():
    ...
    client = AsyncOpenApiClient(base_url="http://api", session=slow_session, retries=1, timeout=0.01)
...
def test_retry_on_500_then_success():
    ...
    client = AsyncOpenApiClient(base_url="http://api", session=flaky_session, retries=4, timeout=1)
