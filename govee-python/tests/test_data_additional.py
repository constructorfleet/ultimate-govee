...
def test_account_configuration_helpers_additional():
    ...
    auth = configuration.govee_authenticated_headers(
        {"clientId": "cid", "accessToken": "tok"}, "5"
    )
...
async def test_receiver_service_bind_and_on_message(caplog):
    ...
    assert any(
        "Device Found" in rec.getMessage() or "Device status" in rec.getMessage()
...
def test_async_http_session_with_fake_httpx(monkeypatch):
    ...
    monkeypatch.setattr(
        async_mod, "httpx", types.SimpleNamespace(AsyncClient=FakeAsyncClient)
    )
...
def test_httpx_session_with_fake_and_fallback(monkeypatch):
    ...
    monkeypatch.setitem(
        __import__("sys").modules, "urllib.request", __import__("urllib.request")
    )
